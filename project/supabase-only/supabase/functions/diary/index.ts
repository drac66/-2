...
Deno.serve(async (req) => {
  ...
  if (req.method === 'GET') {
    const { data, error } = await sb.from('couple_diary')
      .select('*')
      .or(`visibility.eq.both,author.eq.${t.openid}`)
  ...
  const visibility = body?.visibility || 'self';
  if (!content) return json({ success: false, message: 'content cannot be empty', data: null });
  if (!['self', 'both'].includes(visibility)) return json({ success: false, message: 'visibility must be self or both', data: null });

  const { data, error } = await sb.from('couple_diary').insert({ author: t.openid, title, content, visibility }).select('id').single();
  if (error) return json({ success: false, message: error.message, data: null });
  return json({ success: true, message: 'ok', data });
});
